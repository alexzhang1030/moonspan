...
Default builds (`just check` / `just test` / `just build`) compile the gateway without ROS: the rcl bindings are vendored (`rclwebd/src/ros/ffi/bindings.rs`, regenerated by `scripts/generate-rcl-bindings.sh`) and the protocol engine is tested against an in-memory backend. Builds with `--features ros` link the ROS prefix on `ROS_PREFIX` / `AMENT_PREFIX_PATH` (default `/opt/ros/jazzy`) plus the demo types' generated C typesupport, and the `rclwebd` binary requires that feature. Pair the prefix with `RCLWEBD_SUPPORT_ROW`: `J-FT` ↔ Jazzy, `H-FT` ↔ Humble. Committed bindings target Jazzy; the H-FT live compose regenerates them against Humble before linking (`just e2e-h-ft` / CI `e2e-ros-talker-h-ft`). `just ros-test` needs a sourced ROS environment matching those bindings.
