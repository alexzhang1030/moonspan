...
`cdr_mbt` (package `rclmbt/cdr`) implements the [CDR core contract](./cdr.md). The bounded CDR1 reader and writer cover encapsulation, options as network-order `UInt16`, `CdrEndian`, origin-4 alignment, width-exact raw integers, semantic primitives, Char8 strings, ROS legacy wstring, sequence length and borrowed byte sequences, and immutable `CdrNesting` depth tokens. Writer capacity is `min(max_stream_bytes, max_temporary_allocation)` over the full stream including the header; initial Buffer size_hint is header-sized (`WRITER_INITIAL_SIZE_HINT`) with lazy growth under that logical ceiling. M1 targets CDR1 for the authoritative ROS corpus in little and big endian. ROS `wstring` follows the legacy wire profile (`UInt32` count then `count * 4` payload; core decode exact; canonical encode with zero top-level tail slack; corpus completion may accept four-byte zero tail slack after a terminal wstring). XCDR2 stream foundations remain a follow-on surface for later schema work. The codec returns structured public `CdrError` faults (`code`, `offset`, `needed`, `remaining`).
