...
Status: R-D1 and R-D4 are ruled (owner, 2026-08-12); R-D2/R-D3 stand as recommendations. **R0 is executed**: the decision record is [ADR 0010](../adr/0010-restructure-single-rust-core.md), the live plan is [tasks/plan.md](../../tasks/plan.md), and the pre-restructure repository is preserved at tag `pre-restructure`. Baseline commit for all counts: `d6dd478`. Two recorded deviations from the letter of this proposal during R0: the MoonBit stack was deleted in R0 rather than after the Rust corpus gate (the oracle is the contract + corpus + tail-slack, not the retired source, and keeping the MoonBit toolchain pinned in CI for weeks had real cost — git keeps the source), and the valid/malformed protocol fixtures were kept as frozen data consumed by the core's tests instead of being deleted with the generators (they are the single oracle in action).
