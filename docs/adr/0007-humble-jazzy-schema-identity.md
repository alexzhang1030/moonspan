...
Browser runtime, R2WP, gateway caches, fixtures, and recording metadata need a stable schema identity across ROS distros that expose different type-description surfaces. Phase one must qualify two LTS platforms while keeping later distros and Zenoh topologies as explicit follow-on work.
...
- Both distros use `rmw_fastrtps_cpp` as the default and reference support row and qualify `rmw_cyclonedds_cpp`. Each distro/RMW combination is an independent support row.
...
- Humble bundles use the independent identity scheme `moonspan-schema-v1`. The value is the SHA-256 digest of the deterministic canonical bundle bytes. That digest keeps the `moonspan-schema-v1` scheme and stays a separate identity from `rep2011-rihs`.
...
- Kilted, Lyrical, and Rolling are later expansion candidates. `rmw_zenoh` and Zenoh router topologies enter later topology experiments and independent qualification stages.

## Rationale

- Humble and Jazzy give two LTS baselines with documented Ubuntu host targets and broad robotics adoption.
- Independent distro/RMW support rows keep Fast DDS and Cyclone DDS evidence separable while sharing one R2WP and SDK contract surface.
- `rep2011-rihs` matches Jazzy's native type-description and type-hash path.
- `moonspan-schema-v1` supplies a deterministic identity for Humble deployments that ship recursive schema bundles with generic serialized interfaces.
...
- Phase-one qualification covers Humble/Jammy and Jazzy/Noble on `amd64` and `arm64` for `rmw_fastrtps_cpp` and `rmw_cyclonedds_cpp` support rows.
...
- M0-04 freezes canonical bundle layout, ordering, hashing, and cross-language fixtures for `moonspan-schema-v1`.
- This decision is reflected in [compatibility](../compatibility.md), [R2WP](../protocol/r2wp.md), [`rclmbt`](../runtime/rclmbt.md), [`rclwebd`](../gateway/rclwebd.md), [support matrix](../support-matrix.md), [validation](../validation.md), and the [implementation plan](../../tasks/plan.md).
...
- A phase-one deployment needs an identity scheme beyond `rep2011-rihs` and `moonspan-schema-v1`.
- Kilted, Lyrical, Rolling, or Zenoh evidence is ready to enter the active support matrix.
