...
| `protocol/**` | [R2WP overview](./protocol/r2wp.md), [R2WP v0 normative](../protocol/r2wp-v0.md), [fixtures README](../protocol/testdata/README.md), [compatibility](./compatibility.md), [validation](./validation.md) |
...
- Run `bun run check` to run `docs:check`, then `protocol-check`, then aggregate `protocol-fixtures:check`, then `protocol-moonbit-fixtures:check`, then `protocol-agree:check` exactly once (local Markdown links/images/anchors/PCR enrollment, R2WP v0 registry/CDDL contract validation, valid_boundary → malformed → sequences → parity once each, MoonBit fixture bridge, then three-language agreement).
...
- Run `bun run protocol-agree` or `just protocol-agree` for the three-language agreement check; `bun run protocol-agree:write` or `just protocol-agree-write` to regenerate `protocol/testdata/agreement/report.json`; `bun run test:protocol-agree` for the focused orchestrator suite.
