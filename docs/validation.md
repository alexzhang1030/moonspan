...
| R2WP fixture foundation (M0-03d–e) | TypeScript fixture surface through M0-03e: valid/boundary (20 entries), malformed (55 fixtures), sequences (13 scenarios / 26 events), and parity (46 shared identities + 20 registry-bound rules). Aggregate write/check order is exactly `valid_boundary → malformed → sequences → parity` once each via `scripts/protocol-fixtures.ts`. Current root `bun run check` chains `docs:check`, `protocol-check`, aggregate `protocol-fixtures:check`, then `protocol-moonbit-fixtures:check`. Accepted local evidence after e1–e3 review Accept: commits `3600ff4`, `63f21df`, `154afb1`; `bun run test:protocol-fixtures` 277/277 across four files; full `bun test` 584/584; `bun run check` and pinned `just check` status=ok under Bun 1.3.14 / Rust 1.97.1 / moonc 0.10.6+80dc50f24 / just 1.50.0; two aggregate writes retain parity SHA-256 `d75d07e46f878be00bb05fd395ccec768ad52950f749cad8b9fcd28a208f80c9`; `git diff --check` clean. Layout and commands: [protocol/testdata/README.md](../protocol/testdata/README.md). Hosted CI run evidence remains pending. |
| R2WP Rust parser (M0-03f) | `rclwebd` reference parser under [`rclwebd/src/protocol/`](../rclwebd/src/protocol/): bootstrap steps 1–9, selected-frame steps 1–16, deterministic CBOR, extension TLVs, all 15 CONTROL kinds with nested CDDL. Consumes all 20 valid entries (including the manifest-driven 64 MiB segment recipe) and all 55 malformed binaries (14 bootstrap / 41 frame) with exact code/name/reason/offset/plane/step; borrowed extension and application payloads. Review Accept commits `9c07b4a`, `cca270c`; `cargo test --locked -p rclwebd` 55 of 55; `cargo fmt --all -- --check`; `cargo clippy --locked -p rclwebd --all-targets -- -D warnings`; full `bun test` 584 of 584; pinned `just check` status=ok; the `rclwebd` normal tree is std only; the `serde_json` dev dependency serves fixture tests. |
| R2WP MoonBit parser (M0-03g) | `rclmbt` reference parser under [`rclmbt/protocol/`](../rclmbt/protocol/): bootstrap steps 1–9, selected-frame steps 1–16, deterministic CBOR, extension TLVs, all 15 CONTROL kinds with nested CDDL. Consumes all 20 valid entries (3 bootstrap binaries, 16 frame binaries, fully materialized 64 MiB segment recipe) and all 55 malformed binaries (14 bootstrap / 41 frame) with exact code/name/reason/offset/plane/step; borrowed extension and application `BytesView` backing; four exact Phase 1 SessionReady rows H-FT / H-CY / J-FT / J-CY; u32 / u64 / i64 header bounds. Review Accept commits `2f7352f` (fixture bridge), `1157138` (bootstrap + deterministic CBOR), `0c5e4d2` (extension TLVs + all 15 CONTROL kinds), `133fd9f` (selected-frame steps 1–16); focused `moon test --frozen --target wasm rclmbt/protocol` 69 of 69; full `bun test` 613 of 613; pinned `just check` status=ok under Bun 1.3.14 / Rust 1.97.1 / moonc 0.10.6+80dc50f24 / just 1.50.0. Cross-language agreement continues through M0-03h. Hosted CI run evidence remains pending. |
