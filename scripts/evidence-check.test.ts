...
function baseReport(
  ...
  return {
    ...
    artifacts: [
      {
        role: "raw",
        path: "docs/evidence/testdata/payloads/smoke-raw.json",
...
describe("evidence contract enums and helpers", () => {
  test("exports exact closed sets and gate mapping", () => {
    ...
    expect(REPORT_ID).toBe("rclweb-qualification-report-v1");
    ...
    expect(GATE_EVIDENCE_LEVELS.R0).toEqual(["foundation"]);
    expect(GATE_EVIDENCE_LEVELS.R1).toEqual(["foundation", "N1"]);
    expect(GATE_EVIDENCE_LEVELS.R2).toEqual(["N1"]);
    expect(GATE_EVIDENCE_LEVELS.R3).toEqual(["N1", "N2"]);
    expect(GATE_EVIDENCE_LEVELS.R4).toEqual(["operations", "security"]);
    ...
    expect(SCHEMA_REL).toBe("docs/evidence/schema/qualification-report-v1.json");
    expect(VALID_DIR_REL).toBe("docs/evidence/testdata/valid");
    expect(REPORTS_DIR_REL).toBe("docs/evidence/reports");
  });
...
describe("evidence document validation", () => {
  ...
  test("gate and evidence level mapping is enforced", () => {
    const report = baseReport({ gate: "R2", evidence_level: "foundation" });
    ...
    const r1Foundation = baseReport({ gate: "R1", evidence_level: "foundation" });
    expect(validateReportDocument(r1Foundation)).toEqual([]);
  });

  test("N1 and N2 require support_row_id", () => {
    const n1 = baseReport({ gate: "R2", evidence_level: "N1" });
...
describe("schema/runtime boundary parity", () => {
  ...
  test("generated schema encodes gate/level mapping and N1/N2 provenance", () => {
    ...
    expect(schema.allOf.length).toBeGreaterThanOrEqual(8);
    ...
    const r0 = schema.allOf.find((clause) => {
      const iff = clause.if as { properties?: { gate?: { const?: string } } };
      return iff?.properties?.gate?.const === "R0";
    });
    expect(
      ((r0!.then as { properties: { evidence_level: { enum: string[] } } }).properties
        .evidence_level.enum),
    ).toEqual(["foundation"]);
...
describe("schema write/check identity", () => {
  ...
  test("schema write and check are identity-stable", async () => {
    const temp = await mkdtemp(path.join(tmpdir(), "rclweb-evidence-schema-"));
    try {
      await mkdir(path.join(temp, "docs/evidence/schema"), { recursive: true });
      await mkdir(path.join(temp, "docs/evidence/testdata/valid"), { recursive: true });
      await mkdir(path.join(temp, "docs/evidence/testdata/payloads"), { recursive: true });
      await mkdir(path.join(temp, "docs/evidence/reports"), { recursive: true });
      const raw = await Bun.file(
        path.join(root, "docs/evidence/testdata/payloads/smoke-raw.json"),
      ).arrayBuffer();
      await writeFile(
        path.join(temp, "docs/evidence/testdata/payloads/smoke-raw.json"),
        new Uint8Array(raw),
      );
      await writeFile(
        path.join(temp, "docs/evidence/testdata/valid/smoke-foundation.json"),
        await Bun.file(path.join(root, "docs/evidence/testdata/valid/smoke-foundation.json")).text(),
      );
  ...
  test("schema drift is diagnosed", async () => {
    const temp = await mkdtemp(path.join(tmpdir(), "rclweb-evidence-schema-drift-"));
    try {
      await mkdir(path.join(temp, "docs/evidence/schema"), { recursive: true });
  ...
  test("schema corruption invalid UTF-8 is diagnosed", async () => {
    const temp = await mkdtemp(path.join(tmpdir(), "rclweb-evidence-schema-utf8-"));
    try {
      await mkdir(path.join(temp, "docs/evidence/schema"), { recursive: true });
  ...
  test("writeSchema rejects existing symlink leaf with stable error", async () => {
    const temp = await mkdtemp(path.join(tmpdir(), "rclweb-evidence-schema-symlink-"));
    try {
      await mkdir(path.join(temp, "docs/evidence/schema"), { recursive: true });
  ...
  test("writeSchema rejects existing directory leaf with stable error", async () => {
    const temp = await mkdtemp(path.join(tmpdir(), "rclweb-evidence-schema-dir-"));
    try {
      await mkdir(path.join(temp, "docs/evidence/schema/qualification-report-v1.json"), {
        recursive: true,
      });
...
describe("repository corpus", () => {
  test("committed valid corpus, gate reports, and schema pass checkEvidence", async () => {
    const result = await checkEvidence(root);
    expect(result.ok).toBe(true);
    expect(result.reports).toBeGreaterThanOrEqual(4);
    expect(result.summary).toContain("fixtures=1");
    expect(result.summary).toContain("gate_reports=3");
  });

  test("committed smoke report verifies artifact integrity", async () => {
    const diags = await validateReportFile(
      root,
      "docs/evidence/testdata/valid/smoke-foundation.json",
    );
    expect(diags).toEqual([]);
  });

  test("committed gate reports verify artifact integrity", async () => {
    for (const name of [
      "r1-walking-skeleton.json",
      "r2-04-perf-baseline.json",
      "r3-03-h-ft-live.json",
    ]) {
      const diags = await validateReportFile(root, `${REPORTS_DIR_REL}/${name}`);
      expect(diags).toEqual([]);
    }
  });
});
...
describe("filesystem safety and corpus closure", () => {
  ...
  async function seedTempCorpus(): Promise<string> {
    const dir = await mkdtemp(path.join(tmpdir(), "rclweb-evidence-fs-"));
    temps.push(dir);
    await mkdir(path.join(dir, "docs/evidence/schema"), { recursive: true });
    await mkdir(path.join(dir, "docs/evidence/testdata/valid"), { recursive: true });
    await mkdir(path.join(dir, "docs/evidence/testdata/payloads"), { recursive: true });
    await mkdir(path.join(dir, "docs/evidence/reports"), { recursive: true });
    await writeSchema(dir);
    const payload = '{"ok":true}\n';
    await writeFile(path.join(dir, "docs/evidence/testdata/payloads/raw.json"), payload);
    const report = baseReport();
    report.artifacts = [
      {
        role: "raw",
        path: "docs/evidence/testdata/payloads/raw.json",
    ...
    const pretty = stableJsonPretty(report);
    await writeFile(path.join(dir, "docs/evidence/testdata/valid/report.json"), pretty);
    await writeFile(path.join(dir, "docs/evidence/reports/gate.json"), pretty);
    return dir;
  }

  test("extra non-report file in valid corpus is rejected", async () => {
    const dir = await seedTempCorpus();
    await writeFile(path.join(dir, "docs/evidence/testdata/valid/notes.txt"), "nope\n");
  ...
  test("extra directory in valid corpus is rejected", async () => {
    const dir = await seedTempCorpus();
    await mkdir(path.join(dir, "docs/evidence/testdata/valid/extra"));
    const result = await checkEvidence(dir);
    expect(result.ok).toBe(false);
    expect(result.diagnostics.some((d) => d.includes("unexpected directory"))).toBe(true);
  });

  test("empty gate reports directory is rejected", async () => {
    const dir = await seedTempCorpus();
    await rm(path.join(dir, "docs/evidence/reports/gate.json"));
    const result = await checkEvidence(dir);
    expect(result.ok).toBe(false);
    expect(result.diagnostics.some((d) => d.includes("gate reports"))).toBe(true);
  });
  ...
  test("symlink report entry is rejected", async () => {
    ...
    await writeFile(external, "{}\n");
    await rm(path.join(dir, "docs/evidence/testdata/valid/report.json"));
    await symlink(external, path.join(dir, "docs/evidence/testdata/valid/report.json"));
    const result = await checkEvidence(dir);
    expect(result.ok).toBe(false);
  ...
  test("ancestor symlink on valid corpus path is rejected", async () => {
    ...
    await writeFile(
      ...
      await Bun.file(path.join(dir, "docs/evidence/testdata/valid/report.json")).text(),
    );
    await rm(path.join(dir, "docs/evidence/testdata/valid"), { recursive: true, force: true });
    await symlink(realValid, path.join(dir, "docs/evidence/testdata/valid"));
    const result = await checkEvidence(dir);
    expect(result.ok).toBe(false);
    expect(result.diagnostics.some((d) => d.includes("symlink"))).toBe(true);
  });
  ...
  test("artifact symlink is rejected", async () => {
    ...
    await rm(path.join(dir, "docs/evidence/testdata/payloads/raw.json"));
    await symlink(external, path.join(dir, "docs/evidence/testdata/payloads/raw.json"));
    const diags = await validateReportFile(dir, "docs/evidence/testdata/valid/report.json");
    expect(diags.some((d) => d.includes("symlink"))).toBe(true);
  });

  test("hash mismatch and diagnostics are stable sorted", async () => {
    const dir = await seedTempCorpus();
    const reportPath = path.join(dir, "docs/evidence/testdata/valid/report.json");
