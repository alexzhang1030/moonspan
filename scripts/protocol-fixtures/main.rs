...
fn encode_bootstrap_from_source(source: &Value) -> Vec<u8> {
  match source["kind"].as_str().expect("bootstrap kind") {
    "client_hello" => {
      ...
      let hello = ClientHello {
        ...
        transport_capabilities: TransportCapabilities {
          ...
          max_datagram_size: tc.get("maxDatagramSize").and_then(|v| v.as_u64()).map(|n| n as u32),
        },
        ...
        requested_limits: RequestedLimits {
          ...
          max_message_bytes: rl.get("maxMessageBytes").and_then(|v| v.as_u64()).map(|n| n as u32),
    ...
    "server_hello" => {
      ...
      let hello = ServerHello {
        ...
        effective_limits: EffectiveLimits {
          ...
          max_control_payload_bytes: el["maxControlPayloadBytes"].as_u64().unwrap() as u32,
        },
        extension_capabilities: caps,
      };
    ...
    "bootstrap_error" => {
      ...
      let record =
        BootstrapErrorRecord { code: source["code"].as_u64().unwrap() as u8, message, detail };
...
fn process_manifest(
  ...
  let fixtures =
    manifest["fixtures"].as_array_mut().ok_or_else(|| format!("{manifest_rel}: fixtures array"))?;
  ...
  if write {
    if bins_changed {
      ...
      if let Some(updated) = patch_generated_by(&original, generated_by) {
        ...
        return Err(format!("{}: could not patch generated_by in place", manifest_path.display()));
      }
    }
  }
...
fn main() -> ExitCode {
  ...
  let malformed =
    match process_manifest(&root, "protocol/testdata/malformed/manifest.json", write, generated_by)
    {
      Ok(s) => s,
      Err(e) => {
  ...
  let valid = match process_manifest(&root, "protocol/testdata/manifest.json", write, generated_by)
  {
    Ok(s) => s,
    Err(e) => {
      eprintln!("valid: {e}");
