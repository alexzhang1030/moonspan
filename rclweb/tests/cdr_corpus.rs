...
fn decode_all_56_fixtures_against_manifest_values_and_reencode() {
  let manifest = load_manifest();
  let tail = load_tail_slack();
  let tail_by_id: HashMap<&str, &serde_json::Value> =
    tail["fixtures"].as_array().unwrap().iter().map(|f| (f["id"].as_str().unwrap(), f)).collect();
  ...
  for fx in fixtures {
    ...
    r.ensure_complete_with_zero_tail(zero_tail).unwrap_or_else(|e| panic!("{id} zero-tail: {e}"));
