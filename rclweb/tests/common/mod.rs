...
pub fn decode_collections(r: &mut CdrReader<'_>, _n: CdrNesting) -> Collections {
  ...
  let f64_count = r.read_sequence_length(Some(4)).unwrap_or_else(|e| abort("bounded_f64 count", e));
...
pub fn encode_collections(w: &mut CdrWriter, v: &Collections, _n: CdrNesting) {
  ...
  w.write_wstring(&v.bounded_wstring, Some(16)).unwrap_or_else(|e| abort("enc bounded_wstring", e));
}
...
pub fn encode_nested_sample(w: &mut CdrWriter, v: &NestedSample, current: CdrNesting) {
  ...
  let coll_n = w.enter_nested(current).unwrap_or_else(|e| abort("enc NestedSample.Collections", e));
...
pub fn decode_point_cloud2<'a>(r: &mut CdrReader<'a>, parent: CdrNesting) -> PointCloud2<'a> {
  ...
  PointCloud2 { header, height, width, fields, is_bigendian, point_step, row_step, data, is_dense }
}
...
pub fn decode_case<'a>(r: &mut CdrReader<'a>, case_id: &str) -> CorpusValue<'a> {
  ...
  match case_id {
    ...
    "echo_nested_request" => {
      let input_n = r.enter_nested(root).unwrap_or_else(|e| abort("EchoNestedRequest.input", e));
      CorpusValue::EchoNestedRequest { input: decode_nested_sample(r, input_n) }
    }
    "echo_nested_response" => {
      let output_n = r.enter_nested(root).unwrap_or_else(|e| abort("EchoNestedResponse.output", e));
    ...
    "measure_sequence_feedback" => {
      ...
      CorpusValue::MeasureSequenceFeedback { progress, sample: decode_nested_sample(r, sample_n) }
    }
...
pub fn assert_point_cloud2_vs_json(v: &PointCloud2<'_>, j: &Value) {
  ...
  assert_eq!(u64::from(v.header.stamp.nanosec), j["header"]["stamp"]["nanosec"].as_u64().unwrap());
...
pub fn assert_value_vs_json(value: &CorpusValue<'_>, case_id: &str, j: &Value) {
  match (case_id, value) {
    ("primitive_scalars" | "primitive_scalars_big_endian", CorpusValue::PrimitiveScalars(v)) => {
    ...
    ("measure_sequence_feedback", CorpusValue::MeasureSequenceFeedback { progress, sample }) => {
...
pub fn semantic_eq(a: &CorpusValue<'_>, b: &CorpusValue<'_>) -> bool {
  match (a, b) {
    ...
    (CorpusValue::EchoNestedRequest { input: x }, CorpusValue::EchoNestedRequest { input: y }) => {
      semantic_eq(&CorpusValue::NestedSample(x.clone()), &CorpusValue::NestedSample(y.clone()))
    }
    (
