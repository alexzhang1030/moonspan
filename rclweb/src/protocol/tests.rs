...
fn parse_manifest_i64(v: &Value, label: &str) -> i64 {
  ...
  if v.get("$type").and_then(|t| t.as_str()) == Some("bigint") {
    let s = v["value"].as_str().unwrap_or_else(|| panic!("{label}: bigint value must be string"));
...
fn malformed_bootstrap_fixtures_match_expected_oracle() {
  ...
  for entry in fixtures {
    ...
    let path = entry["path"].as_str().unwrap_or_else(|| panic!("{}: missing path", entry["id"]));
    let id = entry["id"].as_str().unwrap();
    let bytes = load_bin(&root, path);
...
fn malformed_frame_fixtures_match_expected_oracle() {
  ...
  for entry in fixtures {
    ...
    let bytes = load_bin(&root, path);
    let expected = &entry["expected"];
    let ctx = entry.get("decoder_context").cloned().unwrap_or(Value::Object(Default::default()));
