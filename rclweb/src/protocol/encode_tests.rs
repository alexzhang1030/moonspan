...
fn valid_frame_fixtures_reencode_byte_identical() {
  ...
  for entry in manifest["fixtures"].as_array().unwrap() {
    ...
    let frame = parse_frame(&bytes, Some(&opts)).unwrap_or_else(|e| panic!("{id}: parse {e:?}"));
    ...
    let payload = match &frame.payload {
      ...
      FramePayload::Control(msg) => {
        let map =
          CborValue::Map(msg.fields.iter().map(|(k, v)| (*k, v.clone())).collect::<Vec<_>>());
        encode_deterministic_cbor(&map).unwrap_or_else(|e| panic!("{id}: control encode {e:?}"))
      }
    };
...
fn deterministic_cbor_boundary_heads_round_trip() {
  ...
  let cases: Vec<CborValue<'static>> = vec![
    ...
    CborValue::Map(vec![(1, CborValue::Unsigned(2)), (29, CborValue::Text(Cow::Borrowed("x")))]),
  ];
  for value in &cases {
...
fn frame_encoder_rejects_oversized_and_misaligned_input() {
  ...
  let control_header =
    FrameHeader { opcode: super::frame::OPCODE_CONTROL_CBOR, channel_id: 0, priority: 0, ..header };
