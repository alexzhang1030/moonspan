...
fn kind_schema(kind: u8) -> Option<Schema> {
  Some(match kind {
    ...
    CONTROL_KIND_GRAPH_SNAPSHOT => fmap(vec![
      ...
      (
        23,
        req(arr(0, GRAPH_ENDPOINTS_MAX, graph_endpoint(), Some(ArrayRule::GraphEndpointsSorted))),
      ),
    ]),
    ...
    CONTROL_KIND_HEARTBEAT => fmap(vec![
      ...
      (2, req(bytes16())),
      (40, req(uint64())),
      (41, opt(arr(0, ALIVE_CHANNELS_MAX, app_channel_id(), Some(ArrayRule::UniqueAscendingUint)))),
    ]),
    CONTROL_KIND_SESSION_RESUME => fmap(vec![
      ...
      (6, req(uint64())),
      (13, req(text_nonempty())),
      (45, req(arr(0, CHANNEL_ACKS_MAX, channel_ack(), Some(ArrayRule::UniqueAscendingChannelId)))),
      (16, req(text_nonempty())),
      (17, req(bytes_cred())),
    ]),
  ...
  fn fifteen_control_kinds_decode_valid_payloads() {
    ...
    for k in 1u8..=15 {
      ...
      let msg = decode_control_message(&bytes).unwrap_or_else(|e| {
        panic!("kind {k} ({}) decode failed: {:?}", CONTROL_KIND_NAMES[(k - 1) as usize], e);
      });
