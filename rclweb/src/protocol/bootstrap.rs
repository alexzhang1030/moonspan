...
fn decode_extension_capabilities(
  ...
  for el in arr {
    let id =
      as_uint_range(el, offset, u64::from(CAPABILITY_ID_MIN), u64::from(CAPABILITY_ID_MAX))? as u16;
    if let Some(p) = prev {
      if id == p {
        return Err(ProtocolError::malformed_bootstrap("unique_violation", offset, 9));
      }
...
mod unit_tests {
  ...
  fn trailing_total_mismatch_step7() {
    let mut bytes = vec![0x52, 0x32, 0x57, 0x50, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01];
