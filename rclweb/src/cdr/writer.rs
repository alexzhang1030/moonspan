...
impl CdrWriter {
  ...
  pub fn write_sequence_length(
    ...
    if let Some(bound) = max_elements
      && length > bound
    {
      return Err(CdrError::bounds_exceeded(field_start, u64::from(length), u64::from(bound)));
    }
    let max_stream = self.limits.max_stream_bytes;
    if (length as usize) > max_stream {
      return Err(CdrError::bounds_exceeded(field_start, u64::from(length), max_stream as u64));
    }
    self.align_and_reserve(4, 4)?;
  ...
  pub fn write_byte_sequence(
    ...
    self.align_and_reserve(4, body_size)?;
    let n_u =
      u32::try_from(n).map_err(|_| CdrError::length_overflow(field_start, 0, rem_cap as u64))?;
