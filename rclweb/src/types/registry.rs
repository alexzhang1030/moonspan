...
impl CdrRepresentation {
    ...
    pub fn from_u16(value: u16) -> Result<Self, SchemaError> {
        match value {
            ...
            other => Err(SchemaError::invalid_schema_key(format!(
                "unknown cdr_representation 0x{other:04x}"
            ))
...
impl SchemaRegistry {
    ...
    pub fn phase1_cached() -> &'static SchemaRegistry {
        ...
        REGISTRY.get_or_init(|| {
            SchemaRegistry::phase1().expect("embedded Phase 1 schema metadata must load")
        })
    }
...
fn validate_type_name_len(type_name: &str) -> Result<(), SchemaError> {
    ...
    if type_name.is_empty() || n > MAX_TYPE_NAME_CHARS {
        return Err(SchemaError::invalid_schema_key(format!(
    ...
    fn into_rep(self) -> Result<CdrRepresentation, SchemaError> {
        match self {
            Self::Name(s) => match s.as_str() {
                ...
                other => Err(SchemaError::invalid_schema_key(format!(
                    "unknown cdr_representation `{other}`"
                ))
                .with_field("cdr_representation")),
            },
...
fn load_identities(builder: &mut SchemaRegistryBuilder) -> Result<(), SchemaError> {
    let file: IdentitiesFile = serde_json::from_str(IDENTITIES_JSON).map_err(|e| {
        SchemaError::schema_input_invalid(format!("identities.json parse failed: {e}"))
    })?;
...
fn load_provenance(builder: &mut SchemaRegistryBuilder) -> Result<(), SchemaError> {
    let file: ProvenanceFile = serde_json::from_str(PROVENANCE_JSON).map_err(|e| {
        SchemaError::schema_input_invalid(format!("provenance.json parse failed: {e}"))
    })?;
...
pub fn lookup_phase1_root_for_open(
    ...
    let scheme = if support_row_id.starts_with('J') {
        SCHEME_REP2011_RIHS
    } else {
