...
impl Worker {
  ...
  fn build_graph_view(&mut self) -> Result<GraphView, BackendError> {
    ...
    for (name, types) in topics {
      let type_name = types.into_iter().next().unwrap_or_else(|| "std_msgs/msg/String".to_owned());
      let mut eid = [0u8; 16];
    ...
    for (name, types) in services {
      let type_name = types.into_iter().next().unwrap_or_else(|| "std_msgs/srv/Empty".to_owned());
      let mut eid = [0u8; 16];
      let n = (index as u64).saturating_add(1);
      eid[8..].copy_from_slice(&n.to_be_bytes());
      endpoints.push(GraphEndpointInfo {
  ...
  fn create_subscription(
    ...
    let Some(msg_ts) = typesupport::message_type_support(&spec.type_name) else {
      return Err(BackendError::new(10, format!("no dynamic typesupport for {}", spec.type_name)));
    };
    let subscription =
      SerializedSubscription::create(&mut self.attachment, &spec.topic, msg_ts.handle, &spec.qos)
  ...
  fn create_publisher(&mut self, spec: &ChannelSpec) -> Result<EntityId, BackendError> {
    ...
    let publisher =
      SerializedPublisher::create(&mut self.attachment, &spec.topic, msg_ts.handle, &spec.qos)
        .map_err(map_rcl_error)?;
    let entity = self.allocate();
  ...
  fn create_client(&mut self, spec: &ChannelSpec) -> Result<EntityId, BackendError> {
    ...
    let client = SerializedClient::create(&mut self.attachment, &spec.topic, service_ts, &spec.qos)
      .map_err(map_rcl_error)?;
    let entity = self.allocate();
  ...
  fn send_service_response(
    ...
    entry.service.send_response(entry.service_ts.response, header, response).map_err(map_rcl_error)
  }
  ...
  fn send_action_feedback(
    ...
    entry.server.publish_feedback(&entry.action_ts, operation_id, payload).map_err(map_rcl_error)
  }
  ...
  fn send_action_result(
    ...
    if let Some(mut header) = pending {
      entry.server.send_result(&entry.action_ts, &mut header, payload).map_err(map_rcl_error)?;
    }
    Ok(())
  }
  ...
  fn resize_wait_set_if_needed(&mut self) -> Result<(), BackendError> {
    ...
    if sub_cap > self.wait_set.subscription_capacity() || svc_cap > self.wait_set.service_capacity()
    {
      ...
      loop {
        match entry.service.take_request(entry.service_ts.request) {
          Ok(Some((header, cdr))) => {
            ...
            let request = ServiceRequest::from_payload(entry.channel_id, operation_id, &cdr);
  ...
  fn pump_one_action_server(&mut self, entity: EntityId) {
        ...
        Ok(Some((mut header, uuid, goal_cdr))) => {
          match entry.server.accept_goal(&entry.action_ts, &mut header, uuid) {
            Ok(handle) => {
              ...
              let inbound = ActionInbound::from_goal_payload(entry.channel_id, uuid, &goal_cdr);
    ...
    loop {
      match entry.server.take_result_request(&entry.action_ts) {
        Ok(Some((header, uuid))) => {
          let Some((ready_cdr, handle, already)) = entry.goals.get_mut(&uuid).map(|slot| {
