...
impl<'a> ConnState<'a> {
  ...
  fn push_app_frame(
    ...
    let extension_area = if let Some(opid) = operation_id {
      let ext =
        R2wpExtension { type_id: OPERATION_ID_EXTENSION_TYPE, critical: true, value: &opid };
  ...
  async fn handle_frame<B: RosBackend>(
    ...
    match frame.opcode {
      OPCODE_CONTROL_CBOR => {
        ...
        self
          .handle_control(msg, &effects, backend, sample_tx, service_tx, action_tx, outcome)
          .await;
      }
      OPCODE_ROS_SAMPLE => {
  ...
  async fn push_graph_snapshot<B: RosBackend>(
    ...
    let snap = control::graph_snapshot(self.config, &control::ZERO_CORRELATION, generation, &view);
    ...
    match msg.kind {
      ...
      CONTROL_KIND_OPEN_CHANNEL => {
        if let Some(channel_id) = effects.channel_opened {
          self
            .open_channel(channel_id, msg, backend, sample_tx, service_tx, action_tx, outcome)
            .await;
        }
      }
  ...
  async fn open_channel<B: RosBackend>(
    ...
    let created = match kind {
      OperationKind::TopicSubscribe => backend.create_subscription(&spec, sample_tx.clone()).await,
      ...
      OperationKind::ActionServer => backend.create_action_server(&spec, action_tx.clone()).await,
    };
    match created {
      Ok(entity) => {
        ...
        let reply = match kind {
          OperationKind::ServiceClient | OperationKind::ServiceServer => {
            control::channel_ready_service_allow(self.config, &correlation, channel_id, priority)
          }
          OperationKind::ActionClient | OperationKind::ActionServer => {
            control::channel_ready_action_allow(self.config, &correlation, channel_id, priority)
          }
        ...
        if let Ok(view) = backend.graph_view().await
          && let Some(endpoint) =
            view.endpoints.iter().find(|ep| ep.name == topic && ep.type_name == type_name)
        {
          self.push_graph_delta_for_endpoint(outcome, endpoint).await;
        }
      }
      Err(err) => {
        let reply = control::channel_ready_error(&correlation, channel_id, err.code, &err.message);
        self.push_control(outcome, &reply);
      }
    }
  }
