...
use rclweb::{
    ...
    Role, Session, TransportCapabilities, encode_client_hello, encode_control_frame, encode_frame,
    parse_bootstrap, parse_frame,
};
use rclwebd::backend::{BackendError, ChannelSpec, EntityId, RosBackend, SubscriptionSample};
use std::borrow::Cow;
...
/// In-memory ROS backend: rejects type names outside `std_msgs/` and
/// `sensor_msgs/` with wire code 10 (schema_unavailable).
#[derive(Default)]
...
impl MockBackend {
    ...
    fn check_type(spec: &ChannelSpec) -> Result<(), BackendError> {
        if spec.type_name.starts_with("std_msgs/") || spec.type_name.starts_with("sensor_msgs/") {
            Ok(())
        } else {
