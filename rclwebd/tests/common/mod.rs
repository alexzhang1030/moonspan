...
use rclweb::{
  BootstrapRecord, BufferCapabilities, CborValue, ClientHello, FrameHeader, RequestedLimits, Role,
  Session, TransportCapabilities, encode_client_hello, encode_control_frame, encode_extension_area,
  encode_frame, parse_bootstrap, parse_frame,
};
use rclwebd::backend::{
...
impl MockBackend {
  ...
  pub fn emit_service_request(
    ...
    sink.try_send(ServiceRequest::from_payload(service.channel_id, operation_id, payload)).is_ok()
  }

  /// Inject an action goal toward an ActionServer entity.
  pub fn emit_action_goal(&self, entity: EntityId, operation_id: [u8; 16], payload: &[u8]) -> bool {
    let inner = self.inner.lock().unwrap();
    ...
    sink
      .try_send(ActionInbound::from_goal_payload(action.channel_id, operation_id, payload))
      .is_ok()
  }
...
impl RosBackend for MockBackend {
  ...
  async fn create_client(&self, spec: &ChannelSpec) -> Result<EntityId, BackendError> {
    ...
    self
      ...
      .unwrap()
      .services
      .insert(entity, MockService { channel_id: spec.channel_id, spec: spec.clone(), sink: None });
    Ok(entity)
  }
  ...
  async fn create_action_client(&self, spec: &ChannelSpec) -> Result<EntityId, BackendError> {
    ...
    self
      ...
      .unwrap()
      .actions
      .insert(entity, MockAction { channel_id: spec.channel_id, spec: spec.clone(), sink: None });
    Ok(entity)
  }
...
impl TestClient {
  ...
  pub async fn recv_raw(&mut self) -> Option<Vec<u8>> {
    ...
    loop {
      let msg = tokio::time::timeout(deadline, self.ws.next()).await.expect("recv timeout")?;
  ...
  pub async fn send_service_request(
    ...
    let ext =
      R2wpExtension { type_id: OPERATION_ID_EXTENSION_TYPE, critical: true, value: &operation_id };
