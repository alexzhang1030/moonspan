...
| `[profile.release]` | thin LTO, `codegen-units = 1`, `strip = "symbols"` | Native gateway binaries. `release-wasm` still inherits then overrides to fat LTO / `panic = abort` / `opt-level = z` (R-D1) |
