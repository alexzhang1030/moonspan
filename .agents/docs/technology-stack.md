...
- **just 1.50.0** (`.just-version`) provides root `toolchain-check`, `protocol-check`, aggregate `protocol-fixtures-check` / `protocol-fixtures-write`, standalone malformed/sequence/parity recipes, `check`, `test`, and `build`. Studio workspace enrollment begins at U0, so those recipes cover currently enrolled mainline workspaces.
...
- Root `bun run check` runs `docs:check`, then `protocol-check`, then aggregate `protocol-fixtures:check`.
...
- Bun manages TypeScript workspaces and scripts; root `bun run check` is docs static check, protocol contract check, then aggregate fixture check; `bun run toolchain-check` is the installed-tool probe; `bun.lock` is tracked.
