...
- **just 1.50.0** (`.just-version`) provides root `toolchain-check`, `protocol-check`, `protocol-fixtures-check`, `protocol-fixtures-write`, `check`, `test`, and `build`. Studio workspace enrollment begins at U0, so those recipes cover mainline workspaces only.
...
- **`scripts/protocol-fixtures.ts`** generates and checks valid/boundary goldens under `protocol/testdata/`. Root entrypoints: `bun run protocol-fixtures:check` / `protocol-fixtures:write`, `just protocol-fixtures-check` / `protocol-fixtures-write`, and focused `bun run test:protocol-fixtures`.
- Root `bun run check` runs `docs:check`, then `protocol-check`, then `protocol-fixtures:check`.
...
- Bun manages TypeScript workspaces and scripts; root `bun run check` is docs static check, protocol contract check, then fixture check; `bun run toolchain-check` is the installed-tool probe; `bun.lock` is tracked.
