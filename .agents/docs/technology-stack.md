...
- **just 1.50.0** (`.just-version`) provides root `toolchain-check`, `protocol-check`, `check`, `test`, and `build`. Studio workspace enrollment begins at U0, so those recipes cover mainline workspaces only.
...
- **`scripts/protocol-check.ts`** validates the frozen R2WP v0 registry and control CDDL. Root entrypoints: `bun run protocol-check`, `just protocol-check`, and focused `bun run test:protocol`. `bun run check` runs `docs:check` then `protocol-check`.
...
- Bun manages TypeScript workspaces and scripts; root `bun run check` is docs static check then protocol contract check; `bun run toolchain-check` is the installed-tool probe; `bun.lock` is tracked.
