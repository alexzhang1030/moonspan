...
R2WP wire version 0 freezes a 12-byte bootstrap prefix plus deterministic CBOR hello payloads, then a fixed 32-byte selected-version frame header with network-byte-order integers, extension TLVs, and CDR or media payloads. Control maps use RFC 8949 core deterministic encoding with unsigned integer keys under [protocol/schema/control-v0.cddl](../../protocol/schema/control-v0.cddl). Numeric registries live in [protocol/registry/r2wp-v0.json](../../protocol/registry/r2wp-v0.json). Normative prose is [protocol/r2wp-v0.md](../../protocol/r2wp-v0.md); encoding rationale is [ADR 0009](../../docs/adr/0009-r2wp-v0-wire-encoding.md); machine validation is [scripts/protocol-check.ts](../../scripts/protocol-check.ts).

Browser-internal TypeScript modules under `sdk/typescript/src/protocol/` implement deterministic CBOR, bootstrap, extension TLVs, all 15 CONTROL kinds, and selected-frame steps 1–16. The Rust parser under [`rclwebd/src/protocol/`](../../rclwebd/src/protocol/) and MoonBit parser under [`rclmbt/protocol/`](../../rclmbt/protocol/) implement the same receiver surfaces against the committed fixtures; MoonBit retains borrowed `BytesView` payloads. Fixture layout and tooling live under [protocol/testdata/](../../protocol/testdata/README.md). The [agreement reference](../../protocol/testdata/agreement/README.md) owns emitter commands and report digests. The [M0-03 completion record](../../docs/milestones/m0-03-r2wp-foundation.md) owns delivery revisions and accepted evidence.
...
- Cargo builds and tests the Rust workspace (`cargo fmt`, locked `clippy -D warnings`, locked `test`, locked `build`). Focused R2WP crate tests use `cargo test --locked -p rclwebd`.
