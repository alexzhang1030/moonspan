...
- Root `bun run check` runs `docs:check`, then `protocol-check`, then aggregate `protocol-fixtures:check`, then `protocol-moonbit-fixtures:check`.
...
- Bun manages TypeScript workspaces and scripts; root `bun run check` is docs static check, protocol contract check, aggregate fixture check, then MoonBit fixture bridge check; `bun run toolchain-check` is the installed-tool probe; `bun.lock` is tracked.
