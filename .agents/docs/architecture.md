...
| TypeScript package | Public rclcpp-shaped API (`init` / `Node`), Worker lifecycle, buffer transfer, telemetry, and reconnect — no protocol parsing. Host, session `connect`, and wasm ABI stay on `rclweb/internal` ([`rclweb`](../../docs/typescript.md)) |
