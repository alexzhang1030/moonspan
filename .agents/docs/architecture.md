...
Detailed system structure lives in [formal architecture](../../docs/architecture.md). Protocol, runtime, and gateway contracts live in [R2WP](../../docs/protocol/r2wp.md), [`rclmbt`](../../docs/runtime/rclmbt.md), and [`rclwebd`](../../docs/gateway/rclwebd.md).
...
Robot edge: rclwebd + schema/policy + ROS C ABI adapter
                 |
                 v
ROS 2 domain through one selected DDS or Zenoh mapping
```
...
|---|---|
| R2WP | Frames, control plane, channel semantics, QoS mapping, errors, versioning |
| `rclmbt` | N2 state and behavior, CDR, type registry, host poll contract |
| Browser SDK | Public typed API, Worker lifecycle, async completion, telemetry |
| `rclwebd` | ROS attachment, schema cache, sessions, scheduling, policy, audit, operations |
| ROS adapter | Versioned serialized C ABI and distro/RMW integration |
...
- Platform expansion enters through [compatibility](../../docs/compatibility.md) and [validation](../../docs/validation.md).
