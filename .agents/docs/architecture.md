...
- Application TypeScript bindings for ROS interfaces are generated from `.msg` / `.srv` / `.action` (`scripts/rosidl-dts.ts`). That path does not emit CDR codecs or accept OMG `.idl`. Topic encode/decode stays on the shipped surface; dynamic projection remains later work.
