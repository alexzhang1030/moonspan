...
- The copy budget is two controllable payload copies end to end, with telemetry counters ([performance contracts](../../docs/architecture.md#performance-contracts)). Those two are the RMW take and the Worker→wasm hop; 0 end-to-end is not a target (network RX plus two heaps). Gateway framing is 0 extra copies: live take writes CDR into a header-prefixed buffer. Host poll copies each WS frame into wasm once (external-ptr ownership transfer); command-only batches stay inline. Wasm→application is 0 copies on the thread that owns wasm; the public `Node` API copies PointCloud2 `data` into an owned message and releases the lease. The I/O Worker copies bulk sample fields (PointCloud2 `data`) and service/action CDR before `postMessage` because wasm memory is not shared with main. The published baseline (`just perf-baseline`) leads with ingest latency, CPU µs/sample, and RSS — copy counts stay structural. Foxglove Bridge and rosbridge comparison: [performance](../../docs/performance.md).
