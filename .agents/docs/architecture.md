...
- Intranet / lab WebTransport is ADR 0011 local-dev TLS plus `RCLWEBD_OFFER_WEBTRANSPORT`. `init("192.168.1.10")` uses WT on a localhost page and WebSocket on a LAN-IP page. Do not ask operators to install a CA. Production PKI is a separate follow-up ([intranet recipe](../../docs/deploy.md#intranet-webtransport)).
