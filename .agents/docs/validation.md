...
Historical evidence stays in version control. A newer accepted run updates the authoritative report location. The machine-readable report contract is `rclweb-qualification-report-v1` under [`docs/evidence/`](../../docs/evidence/README.md); raw `docs/evidence/*.json` files are artifacts, not reports.
