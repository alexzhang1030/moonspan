...
Historical evidence stays in version control. A newer accepted run updates the authoritative report location. Reports under [`docs/evidence/reports/`](../../docs/evidence/reports/) point at raw `docs/evidence/*.json`; the checker only verifies those pointers and the review disposition.
