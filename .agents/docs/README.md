...
| `.github/workflows/ci.yml` | [Validation](../../docs/validation.md); `ros-feature-check` compiles `--features ros --tests` ([gotchas](./gotchas.md#no-ci-lane-compiles-the-ros-feature-tests)); do not wrap cargo tests in Docker ([gotchas](./gotchas.md#do-not-wrap-cargo-tests-in-a-docker-mock-lane)). A conflicted PR never starts `ci` ([gotchas](./gotchas.md#pull_request-ci-does-not-start-on-a-conflicted-pr)) |
