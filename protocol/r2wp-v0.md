...
implementation; later work re-freezes additional parked sections without a
wire-version bump.
...
- Dual-scheme `SchemaKey` identity (`rclweb-schema-v1`, `rep2011-rihs`) and the
  frozen local schema registry used at channel activation (nine corpus
  roots, representation-aware zero-tail).

**Still parked** (design intent in registry/CDDL; semantics may change until
re-frozen): wire schema exchange (`SchemaRequest`, `SchemaAdvertise`,
`SchemaResponse`) until a gateway schema-cache path exercises them; `ClockSync`;
session resume (`SessionResume`, `SessionResumeResult`); and the `MEDIA_CHUNK`,
`RECORDING_CHUNK`, and `ASSET_CHUNK` payloads.
...
([`scripts/protocol-fixtures`](../scripts/protocol-fixtures/)).
...
Machine validation of this package is [`scripts/protocol-check.ts`](../scripts/protocol-check.ts).
...
Schema schemes: **`rep2011-rihs`** (71-byte `RIHS01_` + 64 lowercase hex) and **`rclweb-schema-v1`** (64 lowercase hex). Fast DDS is the reference row per distro; Cyclone DDS and Zenoh are peer first-class rows. Jazzy+ is later expansion only.
...
**SERVICE_SERVER** and **ACTION_SERVER** are valid browser **OpenChannel** roles (mainline browser service/action servers), not graph-only roles. Graph endpoint kinds of the same names remain independent advertisement roles and are not collapsed into OpenChannel.
...
Phase-one source bundle entry encodings: `ROS2_INTERFACE_TEXT`, `ROS2_IDL_TEXT`, `TYPE_DESCRIPTION_JSON`, `TYPE_DESCRIPTION_CBOR`, `OPAQUE_BYTES`. Distinct from CDR/media payload encodings. Canonical ordering, bytes, and hash for `rclweb-schema-v1` are frozen with the [corpus](../conformance/cdr/README.md).
