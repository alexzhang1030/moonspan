...
remains the single version, but only the subset that the R1 walking skeleton exercises
is **normative** until later phases re-freeze the rest against a running implementation:
...
- Control kinds `Authenticate`, `SessionReady`, `OpenChannel`, `ChannelReady`,
  `CloseChannel`, `Heartbeat`, and `Error`.
- Opcodes `CONTROL_CBOR` and `ROS_SAMPLE`.
- The flat and embedded error models.

The remaining sections — graph (`GraphSnapshot`, `GraphDelta`), schema exchange
(`SchemaRequest`, `SchemaAdvertise`, `SchemaResponse`), `ClockSync`, session resume
(`SessionResume`, `SessionResumeResult`), `OPERATION_ID` lifecycle, QoS resolution for
Service and Action, and the `MEDIA_CHUNK`, `RECORDING_CHUNK`, and `ASSET_CHUNK`
payloads — are **parked**: they stay in this document and in the registry and CDDL as
design intent, receivers MUST still reject what the validation order rejects, but their
semantics may change without a wire-version bump until the phase that implements them
re-freezes them (graph and schema in R3, media and recording in R4). Fixture coverage
