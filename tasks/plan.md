...
| Published versions | `rcl-web@0.0.3` (tsdown) is the current npm cut (`0.0.1` source, `0.0.2` first tsdown). `rclweb` / `rclwebd` are `0.0.2` on crates.io. First OIDC automatic publish landed from tag `v0.0.3` ([release](../docs/release.md), [ADR 0016](../docs/adr/0016-oidc-trusted-publish.md)). Fixture crates stay `publish = false`. |
