...
| R4-04 | Active | SDK stabilization, docs, examples, and release — rclcpp-shaped `init`/`Node`, Worker URL, subscribe-chatter on `dist/`, Worker session ops, PointCloud2 and Phase 1 corpus msg/srv/action types, reconnect reopens service/action, Worker telemetry; npm publish / `1.0.0` / D-06 remain follow-ups |
